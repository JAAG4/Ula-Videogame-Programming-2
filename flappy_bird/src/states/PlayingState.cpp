...
void PlayingState::handle_inputs(const sf::Event &event) noexcept
{
    ...
    {
        state_machine->change_state("paused", world, bird, score);
    }
}
...
void PlayingState::render(sf::RenderTarget &target) const noexcept
{
    world->render(target);
    bird->render(target);
    render_text(target, 20, 10, "[Press P to Pause] Score: " + std::to_string(score), Settings::FLAPPY_TEXT_SIZE, "flappy", sf::Color::White);
}